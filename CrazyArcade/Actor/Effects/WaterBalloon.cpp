...
WaterBalloon::WaterBalloon(const Vector2& newPosition, int explosionRange)
...
void WaterBalloon::BeginPlay()
{
	super::BeginPlay();
	ComputeDangerTiles();
}

void WaterBalloon::ComputeDangerTiles()
{
...
void WaterBalloon::Tick(float deltaTime)
{
...
bool WaterBalloon::IsExploded() const
{
	return exploded;
}

void WaterBalloon::Explode(float deltaTime)
{
	...
	if (owner)
	{
		...
		// 4방향 전파 (WaterBalloonExplosion::PropagateInDirection과 동일한 차단 규칙)
		static const Vector2 dirs[4] = {
	...
	if (ownerCharacter)
	{
		auto waterBalloonComp = ownerCharacter->GetComponent<WaterBalloonComponent>();
		if (waterBalloonComp)
			waterBalloonComp->OnWaterBalloonExploded();
	}
